...
def clear():
    ...
    sum = 0
    order = []
    order2 = {'coffee':0, 'latte':o, 'smoothie':0, 'tea':0}
...
def send():
    ...
    ws = wb['Sheet1']
    ws.append([now_dt, name, hp, order2['cpffee'], order2['latte'], order3['smoothie'], order2['tea'], sum])
...
def btn_exit():
    ...
    if name == "":
        ...
        return
    if hp == "":
        tk.messagebox.showerror("확인","휴대폰전호를 입력해주세요!")
        entry2.focus()
        return
...
btn_2.grid(row=0, column=0, padx=10, pady=10)
btn_3.grid(row=0, column=0, padx=10, pady=10)
btn_4.grid(row=0, column=0, padx=10, pady=10)
btn_5.grid(row=0, column=0, padx=10, pady=10)
